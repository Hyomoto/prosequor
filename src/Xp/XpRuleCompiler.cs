...
public static class XpRuleCompiler
{
        ...
        Action<string> warn)
    {
        ...
        foreach (XpRuleJson row in rows)
        {
            ...
            {
                warn(
                    $"[prosequor] XP amount rule '{rule.Id}' on skill '{skillId}' has an amount table without a measure pay channel (resistance / voxels / ingredients) — missing metric always uses amount[0].");
            }
        }
